...
import { FolderMinus, Download, Printer, Search } from 'lucide-react';
import api from '../../../api';
...
const OutOfStockReport = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await api.get('/reports/stock/out-of-stock');
  ...
  return (
    <div className="bg-white p-4 sm:p-6 rounded-xl border border-slate-200 shadow-sm min-h-screen space-y-6 font-sans">
        ...
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-800 flex items-center gap-2">
            <div className="p-2 bg-rose-50 rounded-lg">
              <FolderMinus className="text-rose-600" size={24} />
            </div>
            Out of Stock Report
          </h1>
          <p className="text-xs sm:text-sm text-gray-500 mt-1 pl-12">
            Alert sheets highlighting depleted units having zero balances.
          </p>
        </div>
      ...
      <div className="bg-rose-50/40 p-4 border border-rose-100 rounded-xl flex flex-wrap items-center gap-4 text-sm">
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50 border-b border-slate-200 text-slate-600 text-xs uppercase font-bold">
              <tr>
                ...
                <th className="p-4 text-center">Last Restocked</th>
            ...
            <tbody className="divide-y divide-slate-100 text-sm">
              {loading ? (
                <tr>
                  <td colSpan="5" className="p-4 text-center text-gray-500 italic">Fetching out of stock report...</td>
                </tr>
              ) : filteredItems.length === 0 ? (
                <tr>
                  <td colSpan="5" className="p-4 text-center text-gray-500 italic">No out of stock items found.</td>
                </tr>
              ) : (
