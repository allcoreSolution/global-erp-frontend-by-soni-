...
import { Sliders, Download, Printer, Search } from 'lucide-react';
import api from '../../../api';
...
const StockAdjustmentReport = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await api.get('/reports/stock/adjustment');
  ...
  return (
    <div className="bg-white p-4 sm:p-6 rounded-xl border border-slate-200 shadow-sm min-h-screen space-y-6 font-sans">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 pb-5">
        <div>
          ...
          <p className="text-xs sm:text-sm text-gray-500 mt-1 pl-12">
            Discrepancy logs reconciliation reports matching physical verification.
          </p>
        </div>
      ...
      <div className="bg-teal-50/40 p-4 border border-teal-100 rounded-xl flex flex-wrap items-center gap-4 text-sm">
        <div className="flex-1 min-w-[200px] relative">
          ...
          <input 
            ...
            className="w-full pl-9 pr-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500/50 focus:border-teal-500 transition-all text-sm"
          />
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="block w-full overflow-x-auto w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 text-sm">
              {loading ? (
                <tr>
                  <td colSpan="7" className="p-4 text-center text-gray-500 italic">Fetching stock adjustment report...</td>
                </tr>
              ) : filteredItems.length === 0 ? (
                <tr>
                  <td colSpan="7" className="p-4 text-center text-gray-500 italic">No adjustment records found.</td>
                </tr>
              ) : (
                filteredItems.map((item, idx) => (
                  <tr key={idx} className="hover:bg-teal-50/30 transition-colors">
                    ...
                    <td className="p-4 text-center">
                      <span className={`px-2 py-1 rounded text-xs font-bold ${item.type === 'Addition' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
                        {item.type}
                      </span>
                    </td>
                    <td className="p-4 text-center font-bold text-gray-800">{item.qty > 0 ? `+${item.qty}` : item.qty}</td>
                    <td className="p-4 text-slate-600">{item.reason}</td>
                    <td className="p-4 text-slate-600">{item.adjustedBy}</td>
                  </tr>
                ))
              )}
            </tbody>
