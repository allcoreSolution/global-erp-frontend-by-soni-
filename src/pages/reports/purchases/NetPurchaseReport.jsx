...
const NetPurchaseReport = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await api.get('/reports/purchases/net-purchase');
          ...
          <table className="block w-full overflow-x-auto w-full text-left border-collapse">
            <thead className="bg-slate-50 border-b border-slate-200 text-slate-600 text-xs uppercase font-bold">
              <tr>
                ...
                <th className="p-4">Gross Purchase</th>
                <th className="p-4">Discount Received</th>
              ...
              {loading ? (
                ...
                data.map((row, idx) => (
                  <tr key={idx} className="hover:bg-indigo-50/30 transition-colors">
                    <td className="p-4">{['amount', 'totalAmount', 'grossPurchase', 'discountReceived', 'returns', 'net', 'netPurchase'].includes('period') ? formatCurrency(row['period']) : row['period']}</td>
                    <td className="p-4">{['amount', 'totalAmount', 'grossPurchase', 'discountReceived', 'returns', 'net', 'netPurchase'].includes('grossPurchase') ? formatCurrency(row['grossPurchase']) : row['grossPurchase']}</td>
                    <td className="p-4">{['amount', 'totalAmount', 'grossPurchase', 'discountReceived', 'returns', 'net', 'netPurchase'].includes('discountReceived') ? formatCurrency(row['discountReceived']) : row['discountReceived']}</td>
                    ...
                    <td className="p-4">{['amount', 'totalAmount', 'grossPurchase', 'discountReceived', 'returns', 'net', 'netPurchase'].includes('netPurchase') ? formatCurrency(row['netPurchase']) : row['netPurchase']}</td>
                  </tr>
                ))
              )}
            </tbody>
...
export default NetPurchaseReport;
