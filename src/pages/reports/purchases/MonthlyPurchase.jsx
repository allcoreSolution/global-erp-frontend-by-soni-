...
const MonthlyPurchase = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await api.get('/reports/purchases/monthly');
              ...
              {loading ? (
                ...
                data.map((row, idx) => (
                  <tr key={idx} className="hover:bg-indigo-50/30 transition-colors">
                    <td className="p-4">{['amount', 'totalAmount', 'grossPurchase', 'discountReceived', 'returns', 'net', 'netPurchase'].includes('month') ? formatCurrency(row['month']) : row['month']}</td>
