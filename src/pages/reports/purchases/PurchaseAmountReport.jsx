...
import { DollarSign, Download, Printer, Filter, Search, ArrowUpDown, TrendingDown } from 'lucide-react';
import api from '../../../api';

const PurchaseAmountReport = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await api.get('/reports/purchases/financial/purchase-amount');
    ...
    <div className="bg-white p-4 sm:p-6 rounded-xl border border-slate-200 shadow-sm min-h-screen space-y-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 pb-5">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-gray-800 flex items-center gap-2">
            ...
            Purchase Amount & Spend
          </h1>
          <p className="text-xs sm:text-sm text-gray-500 mt-1 pl-12">
            Audit statements display gross procurement spendings, vendor invoices, and accounts clearance files.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 sm:grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Amount Paid', val: '₹ 1.8 Cr', color: 'bg-emerald-50 text-emerald-700 border-emerald-200' },
          { label: 'Pending Payables', val: '₹ 60.5 L', color: 'bg-rose-50 text-rose-700 border-rose-200' },
        ].map((stat, i) => (
      ...
      <div className="border border-slate-200 rounded-xl overflow-hidden shadow-sm bg-white">
        ...
        <div className="p-4 border-t border-slate-200 bg-slate-50 flex items-center justify-between text-sm text-slate-600">
          <div>Showing 1 to 5 of 280 entries</div>
...
export default PurchaseAmountReport;
