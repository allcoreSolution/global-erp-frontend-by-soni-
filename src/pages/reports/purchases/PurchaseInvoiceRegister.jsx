...
const PurchaseInvoiceRegister = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await api.get('/reports/purchases/invoice-register');
              ...
              {loading ? (
                ...
                data.map((row, idx) => (
                  <tr key={idx} className="hover:bg-indigo-50/30 transition-colors">
                    ...
                    <td className="p-4">{['amount', 'totalAmount', 'grossPurchase', 'discountReceived', 'returns', 'net', 'netPurchase'].includes('invoiceNo') ? formatCurrency(row['invoiceNo']) : row['invoiceNo']}</td>
                    ...
                    <td className="p-4">{['amount', 'totalAmount', 'grossPurchase', 'discountReceived', 'returns', 'net', 'netPurchase'].includes('totalAmount') ? formatCurrency(row['totalAmount']) : row['totalAmount']}</td>
                    <td className="p-4">{['amount', 'totalAmount', 'grossPurchase', 'discountReceived', 'returns', 'net', 'netPurchase'].includes('status') ? formatCurrency(row['status']) : row['status']}</td>
                  </tr>
                ))
              )}
            </tbody>
