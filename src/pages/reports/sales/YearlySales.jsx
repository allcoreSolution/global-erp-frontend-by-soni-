...
import { TrendingUp, Download, Printer, Filter, ArrowUpDown } from 'lucide-react';
import api from '../../../api';
...
const YearlySales = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await api.get('/reports/sales/yearly');
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 pb-5">
        <div>
          ...
          <p className="text-xs sm:text-sm text-gray-500 mt-1 pl-12">
            Annual business turnover sheets containing consolidated graphics, growth indexing and audits.
          </p>
        </div>
        <div className="flex gap-2 w-full sm:w-auto">
          ...
          <button onClick={() => window.print()} className="flex-1 sm:flex-initial flex items-center justify-center gap-2 px-4 py-2 bg-cyan-600 text-white rounded-lg text-sm font-semibold hover:bg-cyan-700 transition-all shadow-sm">
          ...
          <table className="block w-full overflow-x-auto w-full text-left border-collapse">
            <thead className="bg-slate-50 border-b border-slate-200 text-slate-600 text-xs uppercase font-bold">
              <tr>
                ...
                <th className="p-4 text-right text-cyan-700">Net Amount</th>
            ...
            <tbody className="divide-y divide-slate-100 text-sm">
              {loading ? (
                <tr>
                  <td colSpan="7" className="p-4 text-center text-gray-500 italic">Fetching yearly sales data...</td>
                </tr>
              ) : data.length === 0 ? (
                <tr>
                ...
                data.map((row, idx) => (
                  <tr key={idx} className="hover:bg-slate-50 transition-colors">
                    <td className="p-4 font-semibold text-slate-700">{row.year}</td>
                    ...
                    <td className="p-4 text-right font-bold text-cyan-700 bg-cyan-50/30">{formatCurrency(row.netAmount)}</td>
