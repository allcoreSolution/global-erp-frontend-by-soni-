...
const CategoryWiseSales = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await api.get('/reports/sales/analysis/category-wise');
  ...
  const getStatusStyle = (status) => {
    switch(status) {
      case 'High Margin':
      case 'Growing': return 'bg-emerald-100 text-emerald-700 border-emerald-200';
      case 'Stable': return 'bg-indigo-100 text-indigo-700 border-indigo-200';
  ...
  return (
    <div className="bg-white p-4 sm:p-6 rounded-xl border border-slate-200 shadow-sm min-h-screen space-y-6 font-sans">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 pb-5">
        <div>
          ...
          <p className="text-xs sm:text-sm text-gray-500 mt-1 pl-12">
            Consolidated category charts displaying performance index across product segments.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 sm:grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Categories', val: '12', color: 'bg-blue-50 text-blue-700 border-blue-200' },
          { label: 'Top Category', val: 'Electronics', color: 'bg-emerald-50 text-emerald-700 border-emerald-200' },
          { label: 'Lowest Margin', val: 'Appliances', color: 'bg-amber-50 text-amber-700 border-amber-200' },
          { label: 'Total Revenue', val: '₹ 94.0 L', color: 'bg-indigo-50 text-indigo-700 border-indigo-200', icon: <TrendingUp size={16} className="ml-1 inline-block opacity-70" /> },
        ].map((stat, i) => (
        ...
        <div className="overflow-x-auto">
          <table className="block w-full overflow-x-auto w-full text-left border-collapse">
            <thead className="bg-slate-50 border-b border-slate-200 text-slate-600 text-xs uppercase font-bold">
              <tr>
                <th className="p-4 cursor-pointer hover:bg-slate-100 transition-colors group">Category ID</th>
                <th className="p-4 cursor-pointer hover:bg-slate-100 transition-colors group">Category Name</th>
            ...
            <tbody className="divide-y divide-slate-100 text-sm">
              {loading ? (
                ...
                <tr key={idx} className="hover:bg-indigo-50/30 transition-colors">
                  ...
                  <td className="p-4 text-center font-bold text-slate-700">{item.totalItemsSold}</td>
                  <td className="p-4 text-right font-medium text-slate-600">{formatCurrency(item.cost)}</td>
                  <td className="p-4 text-right font-bold text-emerald-600">{formatCurrency(item.revenue)}</td>
                  <td className="p-4 text-center">
                    <span className="font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded border border-indigo-100">{item.margin}</span>
                  </td>
                  <td className="p-4 text-center">
