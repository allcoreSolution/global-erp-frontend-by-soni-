...
import { Home, Download, Printer, Filter, Search, ArrowUpDown, TrendingUp } from 'lucide-react';
import api from '../../../api';

const WarehouseWiseSales = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await api.get('/reports/sales/analysis/warehouse-wise');
        if (response.data.success) {
          setData(response.data.data);
        }
      } catch (error) {
        console.error("Error fetching warehouse wise sales:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="bg-white p-4 sm:p-6 rounded-xl border border-slate-200 shadow-sm min-h-screen space-y-6 font-sans">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 pb-5">
        <div>
          ...
          <p className="text-xs sm:text-sm text-gray-500 mt-1 pl-12">
            Reconciled invoice values grouped by warehouse distribution sources.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: 'Active Warehouses', val: '3', color: 'bg-blue-50 text-blue-700 border-blue-200' },
          { label: 'Top Dispatcher', val: 'Central WH', color: 'bg-emerald-50 text-emerald-700 border-emerald-200' },
          { label: 'Avg Fulfillment', val: '95%', color: 'bg-amber-50 text-amber-700 border-amber-200' },
          { label: 'Total Value Fulfilled', val: '₹ 4.77 Cr', color: 'bg-indigo-50 text-indigo-700 border-indigo-200', icon: <TrendingUp size={16} className="ml-1 inline-block opacity-70" /> },
        ].map((stat, i) => (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50 border-b border-slate-200 text-slate-600 text-xs uppercase font-bold">
              <tr>
                <th className="p-4 cursor-pointer hover:bg-slate-100 transition-colors group">Warehouse ID</th>
                <th className="p-4 cursor-pointer hover:bg-slate-100 transition-colors group">Warehouse Name</th>
                <th className="p-4 cursor-pointer hover:bg-slate-100 transition-colors group">Location</th>
                <th className="p-4 text-center">Items Dispatched</th>
                <th className="p-4 text-right">Revenue Handled</th>
                <th className="p-4 text-center">Fulfillment Rate</th>
            ...
            <tbody className="divide-y divide-slate-100 text-sm">
              {loading ? (
                ...
                <tr key={idx} className="hover:bg-indigo-50/30 transition-colors">
                  ...
                  <td className="p-4 text-center font-bold text-slate-700">{item.itemsDispatched}</td>
                  <td className="p-4 text-right font-bold text-emerald-600">{formatCurrency(item.revenue)}</td>
                  <td className="p-4 text-center">
                    <span className="font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded border border-indigo-100">{item.fulfillmentRate}</span>
                  </td>
                  <td className="p-4 text-center">
...
export default WarehouseWiseSales;
