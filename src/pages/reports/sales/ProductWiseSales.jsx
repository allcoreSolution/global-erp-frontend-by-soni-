...
const ProductWiseSales = () => {
  ...
  const getStatusStyle = (status) => {
    switch(status) {
      ...
      case 'High Margin': return 'bg-indigo-100 text-indigo-700 border-indigo-200';
  ...
  return (
    <div className="bg-white p-4 sm:p-6 rounded-xl border border-slate-200 shadow-sm min-h-screen space-y-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 pb-5">
        <div>
          ...
          <p className="text-xs sm:text-sm text-gray-500 mt-1 pl-12">
            Deep analytical report tracking revenue, margins and units sold for each individual product.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 sm:grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Products Sold', val: '1,445', color: 'bg-blue-50 text-blue-700 border-blue-200' },
          { label: 'Top Selling Category', val: 'Software', color: 'bg-emerald-50 text-emerald-700 border-emerald-200' },
          { label: 'Avg. Profit Margin', val: '31%', color: 'bg-amber-50 text-amber-700 border-amber-200' },
          { label: 'Total Revenue', val: '₹ 87.7 L', color: 'bg-indigo-50 text-indigo-700 border-indigo-200', icon: <TrendingUp size={16} className="ml-1 inline-block opacity-70" /> },
        ].map((stat, i) => (
              ...
              {loading ? (
                ...
                <tr key={idx} className="hover:bg-indigo-50/30 transition-colors">
                  ...
                  <td className="p-4 text-center">
                    <span className="font-bold text-slate-700">{item.unitsSold}</span>
                  </td>
                  <td className="p-4 text-right font-medium text-slate-600">{formatCurrency(item.avgPrice)}</td>
                  <td className="p-4 text-right font-bold text-slate-800">{formatCurrency(item.grossRevenue)}</td>
                  <td className="p-4 text-center">
