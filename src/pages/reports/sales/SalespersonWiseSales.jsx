...
import { Users, Download, Printer, Filter, Search, ArrowUpDown, TrendingUp } from 'lucide-react';
import api from '../../../api';

const SalespersonWiseSales = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error("Error fetching salesperson wise sales:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const getStatusStyle = (status) => {
    switch(status) {
      ...
      case 'Good': return 'bg-indigo-100 text-indigo-700 border-indigo-200';
    ...
    <div className="bg-white p-4 sm:p-6 rounded-xl border border-slate-200 shadow-sm min-h-screen space-y-6 font-sans">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-100 pb-5">
        <div>
          ...
          <p className="text-xs sm:text-sm text-gray-500 mt-1 pl-12">
            Sales commission calculations, targets vs achievements, and sales rep performance ratings.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Avg Achievement', val: '81.9%', color: 'bg-amber-50 text-amber-700 border-amber-200' },
          { label: 'Total Team Revenue', val: '₹ 86.0 L', color: 'bg-indigo-50 text-indigo-700 border-indigo-200', icon: <TrendingUp size={16} className="ml-1 inline-block opacity-70" /> },
        ].map((stat, i) => (
          ...
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50 border-b border-slate-200 text-slate-600 text-xs uppercase font-bold">
              <tr>
                ...
                <th className="p-4 cursor-pointer hover:bg-slate-100 transition-colors group">Name</th>
                <th className="p-4 cursor-pointer hover:bg-slate-100 transition-colors group">Branch</th>
            ...
            <tbody className="divide-y divide-slate-100 text-sm">
              {loading ? (
                ...
                <tr key={idx} className="hover:bg-indigo-50/30 transition-colors">
                  ...
                  <td className="p-4 text-right text-slate-500">{formatCurrency(item.target)}</td>
                  <td className="p-4 text-center">
                    <span className="font-bold text-indigo-600 bg-indigo-50 px-2 py-1 rounded border border-indigo-100">{item.targetAchieved}</span>
                  </td>
                  <td className="p-4 text-center">
                    <span className={`px-2.5 py-1 text-xs font-bold rounded-full border ${getStatusStyle(item.rating)}`}>
                      {item.rating}
                    </span>
                  </td>
                </tr>
...
export default SalespersonWiseSales;
