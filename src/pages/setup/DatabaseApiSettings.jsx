...
const DatabaseApiSettings = () => {
  ...
  useEffect(() => {
    api.get('/settings/database-api').then(res => {
  ...
  const handleSave = async (e) => { 
    e.preventDefault(); 
    try {
      await api.put('/settings/database-api', formData);
      alert('Database & API settings saved!');
    ...
    <div className="bg-white p-4 sm:p-6 rounded-lg border border-slate-200/70 shadow-sm min-h-screen space-y-6">
      <div className="flex justify-between items-center border-b pb-4">
        <div>
          ...
          <p className="text-[11px] sm:text-xs text-gray-500">Manage developer API keys and database backups.</p>
        </div>
